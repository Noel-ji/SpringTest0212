...
//이 파일은 시큐리티의 설정을 담당할 파일.(스프링의 환결성정 파일 / 누렁이집(누렁이의 경비초소))
@Configuration // 이 파일이 스프링의 환경 설정파일임을 의미하는 어노테이션.
@EnableWebSecurity // 모든 요청된 URL이 스프링 시큐리티의 제어를 받도록 만드는 어노테이션.
@EnableMethodSecurity(prePostEnabled = true) 
public class SecurityConfig {
	
	// 두 개의 어노테이션 활성화를 통해 시큐리티를 스프링에 등록.
	// 시큐리티에는 SecurityFilterChain 클래스가 동작하면서
	// 모든 요청 URL에 이 클래스가 필터로 적용.
	// 스프링 시큐리티는 오늘 하는 것 말고도 더 세부적으로 분할하고 자세하게
	// 성정하는 것 또한 가능.
	...
	// "/**" - 모든 전체 내용
	// AntPathRequestMatcher -> Ant스타일의 경로 매칭 제공.
	// -> 예를들어 /, /home, /api/** <---모든 경로 포함의 의미
	
	// 시큐리티는 기본적으로 웹의 보안에 위배되는 공격들을 방어하는 것이 이본.
	// 예를 들어 CRSF는 웹 보안 공격중 하나인데 이러한 공격을 방지하기위해
	// CRSF 토큰 세션을 통해 발행함.
	
	// 그렇다면 h2console은 왜 문제가 발생하는가?
	// - h2console은 스프링을 기반으로 실행되는건 맞음 하지만 기본적으로 DBMS이기 때문에
	//   스프링 별도의 존재로 인식.
	// - 별개의 존재다 보니 당연히 스프링과 상관이 없어서 CSRF 토큰을 발행 할 수 없음.
	
	// h2-console을 위해 csrf도 허용해 준다 쳐도 화면 자체를 볼 수가 없는 증상이 발생.
	//  - X-Frame-Option : 클릭개킹 공격을 막기위해 사용.
